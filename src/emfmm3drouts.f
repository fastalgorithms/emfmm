...
c     1            radius,xnodes2,wts2,nquad2,ier)
     ...
     $     center1,
     $     rmlexp(iaddr(1,jbox)),rmlexp(iaddr(2,jbox)),nterms(level1),
     $     center0,
     $     rmlexp(iaddr(1,ibox)),rmlexp(iaddr(2,ibox)),nterms(level0),
...
c
     ...
     $     rmlexp(iaddr(1,jbox)),rmlexp(iaddr(2,jbox)),nterms(level1),
     $     center0,
     $     rmlexp(iaddr(3,ibox)),rmlexp(iaddr(4,ibox)),nterms(level0),
     $     radius,rnodes,rwts,nphi,ntheta)
...
c     1    	      radius,xnodes2,wts2,nquad2,ier)
     ...
     $     rmlexp(iaddr(3,ibox)),rmlexp(iaddr(4,ibox)),nterms(level0),
     $     center1,
     $     rmlexp(iaddr(3,jbox)),rmlexp(iaddr(4,jbox)),nterms(level1),
     $     radius,rnodes,rwts,nphi,ntheta)
...
c
c
        subroutine em3dehsort(n,isource,esort,hsort,evec,hvec)
        implicit real *8 (a-h,o-z)
        dimension isource(1)
        complex *16 esort(3,1),hsort(3,1),evec(3,1),hvec(3,1)
c
ccc        call prinf('isource=*',isource,n)
c        
        do i=1,n
        evec(1,isource(i))=esort(1,i)
        evec(2,isource(i))=esort(2,i)
        evec(3,isource(i))=esort(3,i)
        hvec(1,isource(i))=hsort(1,i)
        hvec(2,isource(i))=hsort(2,i)
        hvec(3,isource(i))=hsort(3,i)
        enddo
c
        return
        end
...
c
c
        subroutine em3dreorder(n,source,isource,cjvec,
     1     cmvec,sourcesort,cjsort,cmsort) 
        implicit real *8 (a-h,o-z)
        dimension source(3,n), sourcesort(3,n), isource(n)
        complex *16 cjvec(3,1), cjsort(3,1)
        complex *16 cmvec(3,1), cmsort(3,1)
c       
        do i = 1,n
        sourcesort(1,i) = source(1,isource(i))
        sourcesort(2,i) = source(2,isource(i))
        sourcesort(3,i) = source(3,isource(i))
        cjsort(1,i) = cjvec(1,isource(i))
        cjsort(2,i) = cjvec(2,isource(i))
        cjsort(3,i) = cjvec(3,isource(i))
        cmsort(1,i) = cmvec(1,isource(i))
        cmsort(2,i) = cmvec(2,isource(i))
        cmsort(3,i) = cmvec(3,isource(i))
        enddo
        return
        end
c
...
c
c
        subroutine em3dpartmpalloc(w,iaddr,nboxes,lmptot,nterms)
        implicit real *8 (a-h,o-z)
        integer box(20)
...
c       
        ...
        iptr=iptr+(nterms(level)+1)*(2*nterms(level)+1)*2
c       
c       ... then, allocate memory for the local expansions
c       
        iaddr(3,ibox)=iptr
        iptr=iptr+(nterms(level)+1)*(2*nterms(level)+1)*2
        iaddr(4,ibox)=iptr
...
c
c
        subroutine em3dsphmpalloc(wlists,iaddr,nboxes,lmptot,nterms)
        implicit real *8 (a-h,o-z)
        integer box(20)
        dimension nterms(0:1)
        dimension iaddr(4,nboxes)
